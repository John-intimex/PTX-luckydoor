<template>
  <div class="news">
    ...
    <ul>
      <li v-for="(n,index) in lastestContents2" :key="index">
        <router-link :to="'/cms/content/'+n.Id">
          <img :src="n.Cover" class="NewsPart" />
        </router-link>
        <p class="news-date">{{n.CreateDate}}</p>
...
export default class PkNews extends Vue {
  ...
  getContentsByCatId () {
      this.$Api.cms.getSubCatContents({ CatId: 40122, IsMobile: false }).then((result) => {
        console.log(result, ' 获取 Category 和所有 SubCategory 的 cms 列表');
        this.lastestContents = result.Data.slice(0, 3);
        this.lastestContents2 = result.Data.slice(3, 6);
      });
    }
  mounted () {
    // this.getNews();
