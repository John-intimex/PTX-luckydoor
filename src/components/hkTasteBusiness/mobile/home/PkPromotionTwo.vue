<template>
<div id="screen">
    <div class="attach_box" v-for="(b,index) in bannerSecondPart" :key="index">
      <div class="attach_one pktwo"   v-bind:style="{'background-image':'url('+b.Image+')'}"></div>
...
@Component
export default class PkPromotionTwo extends Vue {
    bannerFirstPart:any[]=[];
    ...
    getHeaderBannerLst () {
      ...
      this.$Api.promotion.getPromotion('Home', 1).then(result => {
        _this.bannerSecondPart = result.Promotion._BannerList.slice(2, 6);
        _this.bannerSecondPart.forEach(function (item) {
