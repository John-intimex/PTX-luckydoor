<template>
  <div class="in_select_container" :style="styla" v-if="items.length>0">
    <div class="in_select_label" v-if="label">{{label}}</div>
    <div class="in_select_label" v-else>{{items[0].Type ? items[0].Type.Name : ''}}</div>
...
@Component
export default class InsSelectForP extends Vue {
  // data
